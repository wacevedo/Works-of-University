...
namespace CronometroLed
{
    ...
    {      
        ...
        {
            int hor=0, min = 0, seg = -1;
            string mins, segs,hors;
            do
            {
                Console.Clear();
                if (min == 60) { min = 0; hor++; }
                if (seg == 60) { seg = 0; min++; }
                seg++;
                hors = Convert.ToString(hor);
                mins = Convert.ToString(min);
                segs = Convert.ToString(seg);
                WriteLed(hors, 7);
                Console.ForegroundColor = ConsoleColor.Green;
                Console.SetCursorPosition(22,7); Console.Write('█');
                Console.SetCursorPosition(22,10); Console.Write('█');
                Console.ForegroundColor = ConsoleColor.Black;
                WriteLed(mins, 21);               
                Console.ForegroundColor = ConsoleColor.Green;
                Console.SetCursorPosition(45, 7); Console.Write('█');
                Console.SetCursorPosition(45, 10); Console.Write('█');
                Console.ForegroundColor = ConsoleColor.Black;
                WriteLed(segs, 50);
                Thread.Sleep(1000);
            } while (hor <= 99);
        ...
        public static void WriteLed(string cant,int p)
        {  
           int x, y, c, f, g;
           string num;//, cant; 
